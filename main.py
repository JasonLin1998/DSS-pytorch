...
if __name__ == '__main__':
    data_root = os.path.join('D:\Documents\DSS-pytorch-master', 'data')
    ...
    # # -----MSRA-B dataset-----
    image_path = os.path.join(data_root, 'RGBT\\image\\RGB')
    label_path = os.path.join(data_root, 'RGBT\\annotation')
    train_file = os.path.join(data_root, 'RGBT\\train_RGB.txt')
    valid_file = os.path.join(data_root, 'RGBT\\val_RGB.txt')
    test_file = os.path.join(data_root, 'RGBT\\test_RGB.txt')
    ...
    parser.add_argument('--model', type=str, default='./results/run-mist/models/epoch_500.pth')
